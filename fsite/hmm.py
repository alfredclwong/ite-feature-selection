...
class Hmm:
    def __init__(self, n_features, n_classes, lam, hyperparams=default_hyperparams):
        ...
        def selector_loss_fn(sY, S):
            ...
            if self.n_classes:
                sep = [self.n_features + self.n_classes * i for i in range(4)]
                Y_pred = sY[:, sep[0]:sep[1]]
                Y_base = sY[:, sep[1]:sep[2]]
                Y_true = sY[:, sep[2]:sep[3]]
                X = sY[:, sep[3]:]
            ...
            if self.n_classes == 2:
                mmd_loss = - 10 * mmd2(s * X, Y_true)
            else:
                mmd_loss = 0
            mmd_loss = 0

            # Policy gradient (actor-critic)
            loss = (mmd_loss + imitation_loss) * K.sum(s*K.log(S+eps) + (1-s)*K.log(1-S+eps), axis=-1)
            # loss = (mmd_loss) * K.sum(s*K.log(S+eps) + (1-s)*K.log(1-S+eps), axis=-1)
    ...
    def train(self, X, Y, n_iters, X_test=None, Y_test=None, X_val=None, Y_val=None, S_true=None,
              batch_size=128, verbose=False, save_history=False, imbalanced=False):
        ...
        for it in tqdm(range(1, 1+n_iters)):
            ...
            sY = np.concatenate([s, Y_pred, Y_base, Y[idx].reshape(batch_size, -1), X[idx]], axis=-1)
