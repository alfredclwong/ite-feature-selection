...
class MINVASE(INVASE):
    def __init__(self, n_features, n_specialisms, n_classes=0, gamma=0.01, relevant_features=None):
    ...
    def train(self, X, Y, n_iters, X_val=None, Y_val=None, batch_size=32):
        ...
        for it in tqdm(range(n_iters[1])):
            ...
            if (it+1) % (n_iters[1]//10) == 0:
                ...
                for S in self.predict_features(X, threshold=None):
    ...
    def predict(self, X, threshold=0.5):
        ...
        Y = np.mean(np.array([predictor.predict([X, s]) for (predictor, s) in zip(self.predictors, ss)]), axis=-1).T
        return Y
...
if __name__ == '__main__':
    ...
    X_test = X[:N_train]
    Y_test = Y[:N_train]
    minvase.train(X_train, Y_train, [5000, 5000], X_test, Y_test)
    Y_pred = minvase.predict(X_test)
    ss = minvase.predict_features(X_test)
    X_str, Y_str, t_str, Y_pred_str = map(np.array2string, [X, Y, t, Y_pred])
    ss_str = '\n'.join([np.array2string(s.astype(int)) for s in ss])
