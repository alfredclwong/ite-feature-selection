...
class MT_GANITE:
    def __init__(self, m, k):
        ...
        self.sampler = Lambda(lambda s: K.random_binomial((self.batch_size, self.m), s), name="sampler")
        ...
        S = [mt_selector(X) for mt_selector in self.mt_selectors]
        # Generators
        ...
        #ycfs_mt = [self.mt_generators[t]([X, Z]) for t in range(self.k)] # tmp: skip selector
        ...
        # Define models
        self.mt_ganite_scfs = [Model([X, Z, T, yf], T_mt, name=f"MT_GANITE_SCF_{t+1}") for t in range(self.k)]
        ...
        def s_loss_fn(mask, T_mt, T_oh):
            ...
            def loss(y_true, y_pred):
                return complexity_loss + imitation_loss - d_loss_fn()(y_true, y_pred)
            return loss
        ...
        for t in range(self.k):
            ...
            self.mt_ganite_scfs[t].compile(loss=s_loss_fn(masks[t], T_mt, T_oh), optimizer=self.optimizer)
        ...
        self.oh_ganite_cf.compile(loss=ohg_loss_fn(yf, T, Ycf_oh), optimizer=self.optimizer)
    ...
    def train(self, X_train, T_train, yf_train):
        ...
        idxs = [np.where(ts==t)[0] for t in range(self.k)]
        for it in tqdm(range(self.n_iters)):
            # Train discriminator on both mt and oh generators
            for i in range(self.gd_train_ratio):
                # mt generator
                X, Z, T, yf = get_batch()
                Ycf_mt = np.concatenate([self.mt_generators[t].predict([X, Z]) for t in range(self.k)], axis=1)
                Ym_mt = (T.T * yf).T + (1-T) * Ycf_mt
                d_loss_mtg = self.discriminator.train_on_batch([X, Ym_mt], T)

                # oh generator
                X, Z, T, yf = get_batch()
                Ycf_oh = self.oh_generator.predict([X, Z, T, yf])
                Ym_oh = (T.T * yf).T + (1-T) * Ycf_oh
                d_loss_ohg = self.discriminator.train_on_batch([X, Ym_oh], T)
            ...
            for t in range(self.k):
                ...
                T = T_train[idx]
                yf = yf_train[idx]
                mtg_loss = self.mt_ganite_cfs[t].train_on_batch([X, Z, T, yf], T)
                ...
                T = T_train[idx]
                yf = yf_train[idx]
                mts_loss = self.mt_ganite_scfs[t].train_on_batch([X, Z, T, yf], T)

            # oh generator
            X, Z, T, yf = get_batch()
            ohg_loss = self.oh_ganite_cf.train_on_batch([X, Z, T, yf], T)

            if it % (self.n_iters//10) == 0:
                print(f"Iter: {it:4d}/{self.n_iters:4d}")
                print(f"d_loss_mtg: {d_loss_mtg:2.4f} mts_loss: {mts_loss:2.4f} mtg_loss: {mtg_loss:2.4f}")
                print(f"d_loss_ohg: {d_loss_ohg:2.4f} ohg_loss: {ohg_loss:2.4f}")
    ...
    def build_selectors(self): # todo init uniform: zeros?
        ...
        return [Model(X, S[t], name=f"MT_Selector_{t+1}") for t in range(self.k)]
